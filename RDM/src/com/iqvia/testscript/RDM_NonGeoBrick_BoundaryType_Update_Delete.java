...
public class RDM_NonGeoBrick_BoundaryType_Update_Delete {

	//Starting the report generation
	String currentTime = UtilLib.systemTimeStamp();
	public static UtilLib util = new UtilLib();
	public static String TestCase_Name = "RDM_NonGeoBrick_BoundaryType_Update_Delete";
	...
	public void reportconfig() throws SAXException, IOException, ParserConfigurationException {
		...
		report.config().documentTitle("RDM Non Geo Brick Boundary Type update and delete");
		report.config().reportHeadline("RDM Non Geo Brick Boundary Type update and delete");
	}

	@Test
	public void rdm_NonGeoBrick_BoundaryType_Update_Delete() throws Exception{


		logger = report.startTest("RDM Non Geo Brick Boundary Code update & delete scenario, technical view and kafka verification");
		...
		/************* Non Geo Brick Boundary update and delete **************************/
		ApplicationUtility.RDM_Login(report, logger, UrlLaunch, Username_property, Password_property);
		ApplicationUtility.RDM_BoundaryType_UpdateAndDelete(report, logger);
		ApplicationUtility.RDM_Logout(report, logger);
		
		/******** Technical view *****************/
		ApplicationUtility.RDM_Login(report, logger, UrlLaunch, Username_property, Password_property);
		ApplicationUtility.RDM_TechnicalView_Validation_Update_NonGeoBrick(report, logger);
		ApplicationUtility.RDM_Logout(report, logger);
